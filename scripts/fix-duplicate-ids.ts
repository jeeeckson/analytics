...
function generateTransactionId(existingIds: Set<string>): string {
  ...
  do {
    ...
    for (let i = 0; i < 16; i++) {
      const randomIndex = randomBytes[i % 8] % chars.length;
...
function main() {
  ...
  const originalTransactions = allData.slice(0, 299);
  console.log(`Keeping first ${originalTransactions.length} original transactions`);
  ...
  console.log('Generating 301 new transactions with unique IDs...');
  for (let i = 0; i < 301; i++) {
    ...
    if ((i + 1) % 50 === 0) {
      console.log(`Generated ${i + 1}/301 transactions...`);
    }
  }
  ...
  console.log(`  Approved: ${approvedCount} (${(approvedCount / newTransactions.length * 100).toFixed(1)}%)`);
  console.log(`  Declined: ${declinedCount} (${(declinedCount / newTransactions.length * 100).toFixed(1)}%)`);
}
